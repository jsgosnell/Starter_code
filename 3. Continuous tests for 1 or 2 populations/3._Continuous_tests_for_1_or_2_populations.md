...
3.  Seven observers were shown, for a brief period, a grill with 161
    ...
    hypothesis that the mean of the population is 161 flies
...
6.  Using data from Australian athletes
    ...
    bootstrapping techniques. Data at sport \<-
    read.table(“<http://www.statsci.org/data/oz/ais.txt>”, header = T)

7.  The following data are human blood clotting times (in minutes) of
    individuals given one of two different drugs. Test the hypothesis
    that the mean clotting times are equal for the two groups
...
Note you’ll need to cast this dataframe into a dataframe with a column
...
10. Metabolic rates were measured before and after administering a drug.
...
12. Endurance of the above animals was also measured. Data is available
    at
...
14. Using data from Australian athletes
    (<http://www.statsci.org/data/oz/ais.html> for details), determine
    if the average male training at the Australian Institute of Sport
    differs in from the average female training at the site in mercury
    concentration (Hg).

15. Laureysens et al. (2004) measured metal content in the wood of 13
...
16. Data was collected on cholesterol levels following heart attacks.
...
17. First plot how TotalSleep is explained by BrainWt (remember the
    issues with the data). Use ggplot2 to plot the relationship.

18. Next color code each plot point by whether or not its a primate. In
...
<!-- end list -->
  ...
  - Develop a properly-labelled bar graph with error bars to explore how
    total sleep changes with Primate (relabeled as yes/no as
    Primate/Non-Primate; note there are multiple ways to do this\!) –
    use a 95% confidence interval for the bar Predation risk (as a
    factor\!) – use 1 standard error for the bar. Note the difference\!
